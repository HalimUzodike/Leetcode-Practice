...
class Solution:
  def leastInterval(self, tasks: List[str], n: int) -> int:
    ...
    max_frequency = max(task_count.values())  # The max frequency of tasks

    tasks_with_max_freq = list(task_count.values()).count(max_frequency)

    minimum_interval = (max_frequency - 1) * (n + 1) + tasks_with_max_freq  

    return max(minimum_interval, len(tasks))